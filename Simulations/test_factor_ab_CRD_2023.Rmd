---
title: "Simulations for Factorial Design arranged as CRD"
...
<!-- The linear model for a two-way factorial: -->
...
```{r Two-way-CRD}
two.way.crd.sim = function(num_A = NULL, num_B = NULL, n.reps = NULL, mu = NULL, e.sd = NULL, .seed) {
  # Simulate data for a two-way CRD design
  # Args:
  #   num_A = number of levels of factor A
  #   num_B = number of levels of factor B
  #   n.reps = number of reps (of each experimental unit)
  #   mu = overall mean
  #   e.sd =  standard deviation for the residual error. Residuals distributed ~Norm(0, e.sd^2)
  ...
  A.mu = rnorm(n = num_A, mean = 3, sd = 2.5)  # 1st level is the control
  # Effects due to the levels of B
  B.mu = rnorm(n = num_B, mean = 3, sd = 2.5)  # 1st level is the control
  ...
  # The simulated data:
  df =
    expand_grid(A = 1:num_A, B = 1:num_B, rep = 1:n.reps) %>%
    mutate(y = map2_dbl(A, B, ~{mu + A.mu[.x] + B.mu[.y] + AB.mu[.x, .y] + rnorm(n = 1, mean = 0, sd = e.sd)})) %>%
    # fewer mistakes will occur if we use words to encode the levels of a factor:
    mutate(A = str_c("A", A), B = str_c("B", B)) %>%
    mutate(across(A:rep, factor))
...
df = future_map(1:trials, function(x) two.way.crd.sim(num_A = sample(4:7,1), # number of trt A varying from 4 to 7 per trial
                                                     num_B = sample(4:7,1), # number of trt B varying from 4 to 7 per trial
                                                     n.reps = sample(3:5,1), # number of replicates varying from 3 to 5 per trial
...
for(i in 1:length(df)){
  ...
  model[[i]] = aov(y ~ A + B + A:B, df[[i]]) # fit the model
  sig.I[[i]]=summary(model[[i]])[1][[1]][[5]][[3]]<=0.05 # check for p-value on Interaction
  sig.A[[i]]=summary(model[[i]])[1][[1]][[5]][[1]]<=0.05 & sig.I[[i]]==FALSE# check for p-value on A 
 }
...
for(i in 1:length(df_I)){
  ...
  df_I[[i]]$degrees_freedom = (df_I[[i]]$n.trt_A)*(df_I[[i]]$n.trt_B)*(df_I[[i]]$n.reps-1) #calculate the degrees of freedom
}
```
...
for(i in 1:length(df_I)){
   ...
   df_I[[i]]$mse_model= summary(model_int[[i]])[1][[1]][[3]][[4]]
   df_I[[i]]$mse_algorithm= Fisher_MSE(mean(msd[[i]]$n,na.rm=TRUE),df_I[[i]]$n.reps,0.05,df_I[[i]]$degrees_freedom)
}
...
for(i in 1:length(df_I)){
   ...
   msd[[i]]=get_MSD(means[[i]]$emmean,means[[i]]$.group)
   df_I[[i]]$mse_algorithm=Tukey_MSE(mean(msd[[i]]$n,na.rm=TRUE),df_I[[i]]$n.reps,0.05,df_I[[i]]$degrees_freedom,(df_I[[i]]$n.trt_A*df_I[[i]]$n.trt_B))
}
...
for(i in 1:length(df_I)){
   ...
   msd[[i]]=get_MSD(means[[i]]$emmean,means[[i]]$.group)
   df_I[[i]]$mse_algorithm=Bonferroni_MSE(mean(msd[[i]]$n,na.rm=TRUE),df_I[[i]]$n.reps,0.05,df_I[[i]]$degrees_freedom,(df_I[[i]]$n.trt_A*df_I[[i]]$n.trt_B))
}
...
for(i in 1:length(df_I)){
   ...
   msd[[i]]=get_MSD(means[[i]]$emmean,means[[i]]$.group)
   df_I[[i]]$mse_algorithm= Sidak_MSE(mean(msd[[i]]$n,na.rm=TRUE),df_I[[i]]$n.reps,0.05,df_I[[i]]$degrees_freedom,(df_I[[i]]$n.trt_A*df_I[[i]]$n.trt_B))
}
...
for(i in 1:length(df_I)){
   ...
   msd[[i]]=get_MSD(means[[i]]$emmean,means[[i]]$.group)
   df_I[[i]]$mse_algorithm= Scheffe_MSE(mean(msd[[i]]$n,na.rm=TRUE),df_I[[i]]$n.reps,0.05,df_I[[i]]$degrees_freedom,(df_I[[i]]$n.trt_A*df_I[[i]]$n.trt_B))
}
...
dt$design="factorial_crd_ab"
save(dt, file = "factorial_crd_ab.RData")
...
for(i in 1:length(df_A)){
  ...
  df_A[[i]]$degrees_freedom = (df_A[[i]]$n.trt_A)*(df_A[[i]]$n.trt_B)*(df_A[[i]]$n.reps-1) #calculate the degrees of freedom
}
...
for(i in 1:length(df_A)){
  ...
  msd[[i]]=get_MSD(means[[i]]$emmean,means[[i]]$.group)
  df_A[[i]]$mse_model= summary(model_A[[i]])[1][[1]][[3]][[4]]
  df_A[[i]]$mse_algorithm= Fisher_MSE_ab(mean(msd[[i]]$n,na.rm=TRUE),df_A[[i]]$n.reps,0.05,df_A[[i]]$degrees_freedom,df_A[[i]]$n.trt_B)
}
...
for(i in 1:length(df_A)){
  ...
  msd[[i]]=get_MSD(means[[i]]$emmean,means[[i]]$.group)
  df_A[[i]]$mse_algorithm=Tukey_MSE_ab(mean(msd[[i]]$n,na.rm=TRUE),df_A[[i]]$n.reps,0.05,df_A[[i]]$degrees_freedom,df_A[[i]]$n.trt_A,df_A[[i]]$n.trt_B)
}
...
for(i in 1:length(df_A)){
  ...
  msd[[i]]=get_MSD(means[[i]]$emmean,means[[i]]$.group)
  df_A[[i]]$mse_algorithm=Bonferroni_MSE_ab(mean(msd[[i]]$n,na.rm=TRUE),df_A[[i]]$n.reps,0.05,df_A[[i]]$degrees_freedom,df_A[[i]]$n.trt_A,df_A[[i]]$n.trt_B)
}
...
for(i in 1:length(df_A)){
  ...
  msd[[i]]=get_MSD(means[[i]]$emmean,means[[i]]$.group)
  df_A[[i]]$mse_algorithm=Sidak_MSE_ab(mean(msd[[i]]$n,na.rm=TRUE),df_A[[i]]$n.reps,0.05,df_A[[i]]$degrees_freedom,df_A[[i]]$n.trt_A,df_A[[i]]$n.trt_B)
}
...
for(i in 1:length(df_A)){
  ...
  msd[[i]]=get_MSD(means[[i]]$emmean,means[[i]]$.group)
  df_A[[i]]$mse_algorithm=Scheffe_MSE_ab(mean(msd[[i]]$n,na.rm=TRUE),df_A[[i]]$n.reps,0.05,df_A[[i]]$degrees_freedom,df_A[[i]]$n.trt_A,df_A[[i]]$n.trt_B)
}
...
dt=rbind(dt_bon_ab,dt_fisher_ab,dt_sch_ab,dt_sid_ab,dt_tukey_ab)
dt$design="factorial_crd_a"
save(dt, file = "factorial_crd_a.RData")
