---
title: "Simulations for Split plot Design arranged as CRD"
...
```{r simple-split-plot}
split.plot.sim = function(num_rep = NULL, num_A = NULL, num_B = NULL, mu = NULL, e.wp = NULL, e.sp = NULL, .seed) {
  # Simulate data for a split plot design
  # Args:
  #   num_rep = number of reps of the incomplete blocks
 ...
 df <-
    ...
    expand_grid(rep = 1:num_rep, A = 1:num_A) %>%
    ...
    mutate(y = pmap_dbl(list(A, B, wp), function(.A, .B, .wp) {
      mu +                                       # overall mean
    ...
    dplyr::select(-wp) %>%
    # Define the factors:
    mutate(rep = str_c("rep", rep), A = str_c("A", A), B = str_c("B", B)) %>%
    mutate(across(rep:B, factor))
...
df = future_map(1:trials, function(x) split.plot.sim(num_rep = sample(3:5,1), # replicates
                                                     num_A = sample(4:7,1), # main plot
                                                     num_B = sample(4:7,1), # subplot
                                                     mu = rgamma(1,shape = 60.2,rate = 1),
                                                     e.wp = 3,
                                                     e.sp = 6, 
...
for(i in 1:length(df)){
  ...
  contrasts(df[[i]]$rep) = contr.sum
  model[[i]] = lmerTest::lmer(y ~ A + B + A:B + (1|A:rep), df[[i]],REML =T,control=lmerControl(check.conv.singular = .makeCC(action = "ignore",  tol = 1e-4))) # fit the model
  sig.I[[i]] = anova(model[[i]],ddf="Kenward-Roger",type="3")[6][[1]][3]<=0.05
  sig.B[[i]] = anova(model[[i]],ddf="Kenward-Roger",type="3")[6][[1]][2]<=0.05 & sig.I[[i]]==FALSE
}  
...
for(i in 1:length(df_I)){
  df_I[[i]]$n.reps = nlevels(df_I[[i]]$rep) # obtain the number of reps per trial
  df_I[[i]]$n.trt_A = nlevels(df_I[[i]]$A) 
  df_I[[i]]$n.trt_B = nlevels(df_I[[i]]$B)
  df_I[[i]]$degrees_freedom_sp = (df_I[[i]]$n.trt_B-1)*(df_I[[i]]$n.reps-1)*(df_I[[i]]$n.trt_A) #calculate the degrees of freedom
}
...
for(i in 1:length(df_I)){
   ...
   df_I[[i]]$mse_algorithm= Fisher_MSE(mean(msd[[i]]$n,na.rm=TRUE),df_I[[i]]$n.reps,0.05,df_I[[i]]$degrees_freedom_sp)
}
...
for(i in 1:length(df_I)){
   ...
   msd[[i]]=means[[i]][,.(get_MSD_SP(emmean,.group,A))]
   df_I[[i]]$mse_model=as.data.table(VarCorr(model_I[[i]]),comp=c("Variance"))$vcov[2]
   df_I[[i]]$mse_algorithm=Tukey_MSE(mean(msd[[i]]$n,na.rm=TRUE),df_I[[i]]$n.reps,0.05,df_I[[i]]$degrees_freedom_sp,df_I[[i]]$n.trt_B)
}
...
for(i in 1:length(df_I)){
   ...
   msd[[i]]=means[[i]][,.(get_MSD_SP(emmean,.group,A))]
   df_I[[i]]$mse_algorithm=Bonferroni_MSE(mean(msd[[i]]$n,na.rm=TRUE),df_I[[i]]$n.reps,0.05,df_I[[i]]$degrees_freedom_sp,df_I[[i]]$n.trt_B)
}
...
for(i in 1:length(df_I)){
   ...
   msd[[i]]=means[[i]][,.(get_MSD_SP(emmean,.group,A))]
   df_I[[i]]$mse_algorithm= Sidak_MSE(mean(msd[[i]]$n,na.rm=TRUE),df_I[[i]]$n.reps,0.05,df_I[[i]]$degrees_freedom_sp,df_I[[i]]$n.trt_B)
}
...
for(i in 1:length(df_I)){
   ...
   msd[[i]]=means[[i]][,.(get_MSD_SP(emmean,.group,A))]
   df_I[[i]]$mse_algorithm=Scheffe_MSE(mean(msd[[i]]$n,na.rm=TRUE),df_I[[i]]$n.reps,0.05,df_I[[i]]$degrees_freedom_sp,df_I[[i]]$n.trt_B)
}
...
for(i in 1:length(df_B)){
  df_B[[i]]$n.reps = nlevels(df_B[[i]]$rep) # obtain the number of reps per trial
  df_B[[i]]$n.trt_A = nlevels(df_B[[i]]$A) 
  df_B[[i]]$n.trt_B = nlevels(df_B[[i]]$B)
  df_B[[i]]$degrees_freedom_sp = (df_B[[i]]$n.trt_B-1)*(df_B[[i]]$n.reps-1)*(df_B[[i]]$n.trt_A) #calculate the degrees of freedom
}
...
for(i in 1:length(df_B)){
  ...
  df_B[[i]]$mse_algorithm= Fisher_MSE_ab(mean(msd[[i]]$n,na.rm=TRUE),df_B[[i]]$n.reps,0.05,df_B[[i]]$degrees_freedom_sp,df_B[[i]]$n.trt_A)
}
...
for(i in 1:length(df_B)){
  ...
  msd[[i]]=get_MSD(means[[i]]$emmean,means[[i]]$.group)
  df_B[[i]]$mse_model=as.data.table(VarCorr(model_B[[i]]),comp=c("Variance"))$vcov[2]
  df_B[[i]]$mse_algorithm=Tukey_MSE_ab(mean(msd[[i]]$n),df_B[[i]]$n.reps,0.05,df_B[[i]]$degrees_freedom_sp,df_B[[i]]$n.trt_B,df_B[[i]]$n.trt_A)
}
...
for(i in 1:length(df_B)){
  ...
  msd[[i]]=get_MSD(means[[i]]$emmean,means[[i]]$.group)
  df_B[[i]]$mse_model=as.data.table(VarCorr(model_B[[i]]),comp=c("Variance"))$vcov[2]
  df_B[[i]]$mse_algorithm=Bonferroni_MSE_ab(mean(msd[[i]]$n,na.rm=TRUE),df_B[[i]]$n.reps,0.05,df_B[[i]]$degrees_freedom_sp,df_B[[i]]$n.trt_B,df_B[[i]]$n.trt_A)
}
...
for(i in 1:length(df_B)){
  ...
  msd[[i]]=get_MSD(means[[i]]$emmean,means[[i]]$.group)
  df_B[[i]]$mse_model=as.data.table(VarCorr(model_B[[i]]),comp=c("Variance"))$vcov[2]
  df_B[[i]]$mse_algorithm=Sidak_MSE_ab(mean(msd[[i]]$n,na.rm=TRUE),df_B[[i]]$n.reps,0.05,df_B[[i]]$degrees_freedom_sp,df_B[[i]]$n.trt_B,df_B[[i]]$n.trt_A)
}
...
for(i in 1:length(df_B)){
  ...
  msd[[i]]=get_MSD(means[[i]]$emmean,means[[i]]$.group)
  df_B[[i]]$mse_model=as.data.table(VarCorr(model_B[[i]]),comp=c("Variance"))$vcov[2]
  df_B[[i]]$mse_algorithm= Scheffe_MSE_ab(mean(msd[[i]]$n,na.rm=TRUE),df_B[[i]]$n.reps,0.05,df_B[[i]]$degrees_freedom_sp,df_B[[i]]$n.trt_B,df_B[[i]]$n.trt_A)
}
...
# IA degrees of freedom

for(i in 1:length(df_A)){
  df_A[[i]]$n.reps = nlevels(df_A[[i]]$rep) # obtain the number of reps per trial
  df_A[[i]]$n.trt_A = nlevels(df_A[[i]]$A) 
  df_A[[i]]$n.trt_B = nlevels(df_A[[i]]$B)
  df_A[[i]]$degrees_freedom_wp = (df_A[[i]]$n.reps-1)*(df_A[[i]]$n.trt_A) #calculate the degrees of freedom
}
...
for(i in 1:length(df_A)){
  ...
  df_A[[i]]$mse_algorithm= Fisher_MSE_ab(mean(msd[[i]]$n,na.rm=TRUE),df_A[[i]]$n.reps,0.05,df_A[[i]]$degrees_freedom_wp,df_A[[i]]$n.trt_B)
}
...
for(i in 1:length(df_A)){
  ...
  msd[[i]]=get_MSD(means[[i]]$emmean,means[[i]]$.group)
  var_sp[[i]]=as.data.table(VarCorr(model_A[[i]]),comp=c("Variance"))
   df_A[[i]]$mse_algorithm=Tukey_MSE_ab(mean(msd[[i]]$n,na.rm=TRUE),df_A[[i]]$n.reps,0.05,df_A[[i]]$degrees_freedom_wp,df_A[[i]]$n.trt_A,df_A[[i]]$n.trt_B)
}
...
for(i in 1:length(df_A)){
  ...
  msd[[i]]=get_MSD(means[[i]]$emmean,means[[i]]$.group)
  var_sp[[i]]=as.data.table(VarCorr(model_A[[i]]),comp=c("Variance"))
  df_A[[i]]$mse_algorithm= Bonferroni_MSE_ab(mean(msd[[i]]$n,na.rm=TRUE),df_A[[i]]$n.reps,0.05,df_A[[i]]$degrees_freedom_wp,df_A[[i]]$n.trt_A,df_A[[i]]$n.trt_B)
}
...
for(i in 1:length(df_A)){
  ...
  msd[[i]]=get_MSD(means[[i]]$emmean,means[[i]]$.group)
  var_sp[[i]]=as.data.table(VarCorr(model_A[[i]]),comp=c("Variance"))
  df_A[[i]]$mse_algorithm=Sidak_MSE_ab(mean(msd[[i]]$n,na.rm=TRUE),df_A[[i]]$n.reps,0.05,df_A[[i]]$degrees_freedom_wp,df_A[[i]]$n.trt_A,df_A[[i]]$n.trt_B)
}
...
for(i in 1:length(df_A)){
  ...
  msd[[i]]=get_MSD(means[[i]]$emmean,means[[i]]$.group)
  var_sp[[i]]=as.data.table(VarCorr(model_A[[i]]),comp=c("Variance"))
  df_A[[i]]$mse_algorithm=Scheffe_MSE_ab(mean(msd[[i]]$n,na.rm=TRUE),df_A[[i]]$n.reps,0.05,df_A[[i]]$degrees_freedom_wp,df_A[[i]]$n.trt_A,df_A[[i]]$n.trt_B)
}
